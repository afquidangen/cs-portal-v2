import { downloadablesRepository } from "@/features/portal/repositories/downloadables.repository"
import { success, error, badRequest } from "@/lib/api-response"
import { uploadFile } from "@/lib/cloudinary"

export const runtime = "nodejs"

export async function GET() {
  try {
    const links = await downloadablesRepository.findAll() as Record<string, unknown>[]
    return success(links)
  } catch (err) {
    return error(err instanceof Error ? err.message : "Unable to fetch downloadables.")
  }
}

export async function POST(request: Request) {
  try {
    const body = await request.json()
    if (!body.label || !body.href) {
      return badRequest("Label and href are required.")
    }

    const doc: Record<string, unknown> = {
      label: body.label,
      href: body.href,
      fileName: body.fileName,
      fileSize: body.fileSize,
    }

    if (typeof doc.href === "string" && doc.href.startsWith("data:")) {
      const result = await uploadFile(doc.href as string, `downloadable-${Date.now()}`, "downloadables")
      doc.href = result.secureUrl
    }

    const created = await downloadablesRepository.create(doc)
    const raw = typeof created === "object" && created !== null && "toObject" in created
      ? (created as { toObject: () => Record<string, unknown> }).toObject()
      : created as Record<string, unknown>
    const link = { ...raw, _id: String(raw._id) }
    return success(link, 201)
  } catch (err) {
    return error(err instanceof Error ? err.message : "Unable to create downloadable.")
  }
}
